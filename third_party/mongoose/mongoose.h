...
#if MG_ARCH == MG_ARCH_ARMCC || MG_ARCH == MG_ARCH_CMSIS_RTOS1 || MG_ARCH == MG_ARCH_CMSIS_RTOS2
...
#if (MG_ARCH == MG_ARCH_CMSIS_RTOS1 || MG_ARCH == MG_ARCH_CMSIS_RTOS2) && !defined MG_ENABLE_RL && (!defined(MG_ENABLE_LWIP) || !MG_ENABLE_LWIP) && (!defined(MG_ENABLE_TCPIP) || !MG_ENABLE_TCPIP)
...
#if (!defined(MG_ENABLE_TCPIP) || !MG_ENABLE_TCPIP) && (!defined(MG_ENABLE_LWIP) || !MG_ENABLE_LWIP) && (!defined(MG_ENABLE_FREERTOS_TCP) || !MG_ENABLE_FREERTOS_TCP)
...
#define MG_SOCK_PENDING(errcode) \
    (((errcode) < 0) && (WSAGetLastError() == WSAEINTR || WSAGetLastError() == WSAEINPROGRESS || WSAGetLastError() == WSAEWOULDBLOCK))
...
#define MG_SOCK_PENDING(errcode) \
    ((errcode) == -pdFREERTOS_ERRNO_EWOULDBLOCK || (errcode) == -pdFREERTOS_ERRNO_EISCONN || (errcode) == -pdFREERTOS_ERRNO_EINPROGRESS || (errcode) == -pdFREERTOS_ERRNO_EAGAIN)
...
#define MG_SOCK_PENDING(errcode) \
    ((errcode) == BSD_EWOULDBLOCK || (errcode) == BSD_EALREADY || (errcode) == BSD_EINPROGRESS)
...
#define MG_TIMER_AUTODELETE 8  // free() timer when done
    ...
    void mg_timer_init(struct mg_timer** head, struct mg_timer* timer, uint64_t milliseconds, unsigned flags, void (*fn)(void*), void* arg);
...
#define MG_LOAD_BE24(p) \
    ((uint32_t)(((uint32_t)MG_U8P(p)[0] << 16U) | ((uint32_t)MG_U8P(p)[1] << 8U) | MG_U8P(p)[2]))
#define MG_LOAD_BE32(p) \
    ((uint32_t)(((uint32_t)MG_U8P(p)[0] << 24U) | ((uint32_t)MG_U8P(p)[1] << 16U) | ((uint32_t)MG_U8P(p)[2] << 8U) | MG_U8P(p)[3]))
...
#if defined(__GNUC__)
#define MG_ARM_DISABLE_IRQ() asm volatile("cpsid i" \
                                          :         \
                                          :         \
                                          : "memory")
#define MG_ARM_ENABLE_IRQ() asm volatile("cpsie i" \
                                         :         \
                                         :         \
                                         : "memory")
#elif defined(__CCRH__)
...
#define GCM_AUTH_FAILURE 0x5555'5555 // authentication failure
    ...
    //
    //
    int mg_aes_gcm_encrypt(unsigned char* output, const unsigned char* input, size_t input_length, const unsigned char* key, const size_t key_len, const unsigned char* iv, const size_t iv_len, unsigned char* aead, size_t aead_len, unsigned char* tag, const size_t tag_len);
...
#elif (MG_UECC_PLATFORM == mg_uecc_arm_thumb2) && (__ARM_ARCH >= 6) && (!defined(__ARM_ARCH_7M__) || !__ARM_ARCH_7M__)
#define MG_UECC_ARM_USE_UMAAL 1
#else
...
#if (MG_UECC_WORD_SIZE != 1) && (MG_UECC_WORD_SIZE != 4) && (MG_UECC_WORD_SIZE != 8)
...
#if ((MG_UECC_PLATFORM == mg_uecc_arm || MG_UECC_PLATFORM == mg_uecc_arm_thumb || MG_UECC_PLATFORM == mg_uecc_arm_thumb2) && (MG_UECC_WORD_SIZE != 4))
...
#define HIGH_BIT_SET            0x8000'0000
...
#define HIGH_BIT_SET            0x8000'0000'0000'0000U
...
#define MG_UECC_ENABLE_VLI_API 0
    ...
    extern "C"
    {
        ...
        /* Sets dest = src. */
        void mg_uecc_vli_set(mg_uecc_word_t* dest, const mg_uecc_word_t* src, wordcount_t num_words);
        ...
        void mg_uecc_vli_mult(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right, wordcount_t num_words);

        /* Computes result = left^2. Result must be 2 * num_words long. */
        void mg_uecc_vli_square(mg_uecc_word_t* result, const mg_uecc_word_t* left, wordcount_t num_words);

        /* Computes result = (left + right) % mod.
           Assumes that left < mod and right < mod, and that result does not overlap
           mod. */
        void mg_uecc_vli_modAdd(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right, const mg_uecc_word_t* mod, wordcount_t num_words);

        /* Computes result = (left - right) % mod.
           Assumes that left < mod and right < mod, and that result does not overlap
           mod. */
        void mg_uecc_vli_modSub(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right, const mg_uecc_word_t* mod, wordcount_t num_words);

        /* Computes result = product % mod, where product is 2N words long.
           Currently only designed to work for mod == curve->p or curve_n. */
        void mg_uecc_vli_mmod(mg_uecc_word_t* result, mg_uecc_word_t* product, const mg_uecc_word_t* mod, wordcount_t num_words);
        ...
        /* Computes result = (left * right) % mod.
           Currently only designed to work for mod == curve->p or curve_n. */
        void mg_uecc_vli_modMult(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right, const mg_uecc_word_t* mod, wordcount_t num_words);
        ...
        /* Computes result = left^2 % mod.
           Currently only designed to work for mod == curve->p or curve_n. */
        void mg_uecc_vli_modSquare(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* mod, wordcount_t num_words);
        ...
        void mg_uecc_vli_modInv(mg_uecc_word_t* result, const mg_uecc_word_t* input, const mg_uecc_word_t* mod, wordcount_t num_words);
        ...
        void mg_uecc_vli_nativeToBytes(uint8_t* bytes, int num_bytes, const mg_uecc_word_t* native);
...
#define TLS_RSA_H
    ...
    void mg_http_serve_file(struct mg_connection*, struct mg_http_message* hm, const char* path, const struct mg_http_serve_opts*);
...
#define WEBSOCKET_OP_PONG     10
    ...
    struct mg_connection* mg_ws_connect(struct mg_mgr*, const char* url, mg_event_handler_t fn, void* fn_data, const char* fmt, ...);
...
#define MQTT_PROP_SHARED_SUBSCRIPTION_AVAILABLE     0x2A
    ...
    struct mg_connection* mg_mqtt_connect(struct mg_mgr*, const char* url, const struct mg_mqtt_opts* opts, mg_event_handler_t fn, void* fn_data);
    ...
    size_t mg_mqtt_next_prop(struct mg_mqtt_message*, struct mg_mqtt_prop*, size_t ofs);
...
#define MG_WIFI_BAND_5G 1
    ...
    typedef void (*mg_tcpip_event_handler_t)(struct mg_tcpip_if* ifp, int ev, void* ev_data);
...
#define MG_DRIVER_CLK_FREQ 100'000'000UL
#endif
